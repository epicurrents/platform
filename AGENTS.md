...
- Two container runtimes are supported on the distribution path. The generated start.sh and the bundled [scripts/update.sh](scripts/update.sh) detect Docker Engine or Podman and build a single `COMPOSE` array to match; every later call goes through that array, which is what keeps a second runtime a preflight concern rather than a rewrite. Keep it that way — a bare `docker` call added below the detection breaks Podman silently, because compose would still work and only that one call would fail. Detection reads the runtime name out of the version string rather than trusting the command name, since `podman-docker` installs a `docker` that is Podman. Podman runs **rootful** (`sudo -E podman compose`): every service declares `user: "1000:1000"` against a bind mount, and only a rootful runtime maps that uid to the deployment's owner. The checkout path ([scripts/bootstrap-podman.sh](scripts/bootstrap-podman.sh)) has *not* been brought level — see the ROADMAP entry before relying on it.
- [scripts/bootstrap.sh](scripts/bootstrap.sh) installs Docker on a fresh Ubuntu VM. Its Podman sibling and the packaged prepare-host.sh are both Debian/Ubuntu only; a Red Hat recipient does the host preparation by hand.
