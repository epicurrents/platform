...
**Account administration** is the `/admin/` routes in the SPA, client to `/api/v1/user/admin/`; the Django admin is retired and there is no `admin/` mount. Never add an account-deletion control to any surface — `erase_user` is the sanctioned path because it also unlinks owned recording and media files, which FK cascade never does. See [frontend/README.md → Administration](frontend/README.md#administration).

### Project roles render from the wire, never from project frontend code

Core knows the role *mechanism* exists — a registry, an endpoint listing what is registered, a selector on the group form, badges on the account page. Core must never know a role's *meaning*. No role key, value or label may appear anywhere in the frontend, tests and mock fixtures included; use obviously fictional names in fixtures so a real one cannot quietly become load-bearing.

The consequence is that **roles need no project-supplied frontend code and must not grow an extension point** — no entry in [frontend/src/projects/types.ts](frontend/src/projects/types.ts), no project component, no registration call. `GET /api/v1/user/admin/roles` is the sole authority for which selectors render and what goes in them, read at runtime, so a deployment running an unknown project gets working role management with no frontend change. Wanting a project hook here means something has gone wrong.

**The padded-map clear** is the trap to know before touching the group form. `GroupDetailOut.roles` carries an entry for every registered key including the nulls, and `PATCH /admin/groups/{id}` leaves an absent key untouched while reading an explicit `null` as "clear this role". Seeding a reactive form object from the group payload and submitting it back wholesale sends exactly the padded map that clears every role the form did not render — and it is harmless right up until the roles call fails or returns stale data, which is when a silent clear is least welcome. Build the body from the selectors actually rendered, with `rolesPayload` in [frontend/src/api/admin.ts](frontend/src/api/admin.ts); serialise "no role" as `null`, never an empty string, which is not among a provider's declared choices and returns a 400. Regression coverage in [frontend/src/api/admin.test.ts](frontend/src/api/admin.test.ts) asserts on the request body, because a test that checks the resulting state passes in the configuration where the bug is dormant.
...
A read grant carrying `apply_middleware` de-identifies the bytes a caller receives: the fixed EDF header loses its patient identification and the annotation text inside the signal file is replaced by the mandatory timekeeping records. The same annotations also exist as `Event`, `Label` and `Annotation` rows, and serialising those without regard to the grant returns in JSON exactly what the pipeline strips out of the file.

**A caller whose read terms carry `apply_middleware` receives no annotation text they did not write** — `Event.name` and `value`, `Label.name` and `value`, `Annotation.content`. Timing, hashes and author ids are unaffected, and so are a classification code's `standard` and `value` — a code's value is checked against a registered vocabulary, so it names a concept rather than describing a subject. A code's free-form `meta` is the one part nobody validates, so it follows the rule wherever it hangs, including on an interruption, whose own fields are a start and a duration and carry nothing to withhold. Two exemptions, both narrow. The caller's own rows, because a rater reading under a de-identifying grant must keep reading their own work. And machine-produced rows registered through `register_exempt_rows` — [compute](compute/redaction.py) declares an analysis run's findings, which are computed from the already de-identified signal rather than transcribed from it.

[annotations/redaction.py](annotations/redaction.py) is the single decision site, and the three text-bearing serialisers in [annotations/api/v1/ninja.py](annotations/api/v1/ninja.py) take `withhold_text` as a required keyword argument, so a new call site cannot serialise text without deciding. Two endpoints in [recordings/api/v1/ninja.py](recordings/api/v1/ninja.py) serialise these rows through serialisers of their own and call the same helper: the time-slice response and the annotation-bundle listing. The rule needs the *terms*, not a boolean: a surface that resolves access with `can_read_object` has already discarded what it needs, so use `get_read_access_result` or `get_federated_read_access_result` and keep the result. A federated peer authors nothing locally, so under a sanitising grant it sees no text except machine-produced findings, whose exemption holds whoever is asking.

Exporting across annotators is outside this rule and deliberately so — that path is a role, not a grant. Which role is the deployment's choice: `ANNOTATION_EXPORT_ALL_ANNOTATORS_REQUIRES_SUPERUSER` defaults to reserving it for superusers, because the export reads clinical annotation text rather than administration data and is the one path that answers past this rule. A deployment whose staff tier is its research-coordinator tier turns the setting off. `annotations.export.can_export_all_annotators` is the single derivation; the endpoint and the queryset scoping both call it, rather than each deciding.
...
**A distribution carries prebuilt frontend bundles, and they must belong to the project it activates.** `VITE_PROJECT` and `VITE_PLUGINS` are baked into the SPA at build time, so `frontend/dist` on a checkout with a project active *is* that project's UI with nothing in the output saying so — and the packager copies it verbatim. Every build therefore writes `frontend/dist/build-info.json`, and [scripts/make-bootstrap-fixture.sh](scripts/make-bootstrap-fixture.sh) refuses a package whose bundle names a project it does not carry (a blank bundle is the base UI and goes anywhere). The per-project viewer overlays under `viewer-dist/<project>/` are filtered the same way, and the package's `.env.example` is written from what the package carries rather than inherited from the builder's tree. Do not remove the stamp or weaken the check to get a package out: the failure it prevents is silent in every direction — the package starts, the UI offers routes into API mounts that are not there, and a private project's name ships to whoever received it.
...
|---|---|---|
| [annotations/redaction.py](annotations/redaction.py) | Annotation-text sanitisation — decides whose annotation text a caller may read under a grant carrying `apply_middleware`. A surface that serialises `Event` / `Label` / `Annotation` text without consulting it hands back in JSON precisely what the byte pipeline removes from the file, and no byte-level test can see that happen. The exemption registry is part of the surface: widening it past machine-produced rows turns the rule off for whatever is added. | [annotations/tests/test_text_redaction.py](annotations/tests/test_text_redaction.py) — withholding per surface under a sanitising grant, the own-row and machine-produced exemptions, raw-grant and superuser passthrough, the time-slice and bundle endpoints, and a source scan that every serialiser call passes `withhold_text=`; [recordings/tests/test_federation.py](recordings/tests/test_federation.py) — the federated half, where a peer owns no row and the machine exemption still applies. |
...
| [federation/auth.py](federation/auth.py) | Federated authentication surface — JWT signature/alg/exp/iat/aud verification, request binding (`htm` / `htp` / `bnd`), replay (`jti`) cache, SSRF guard, strict TLS context, `is_trusted` peer gate, local-key consistency. Silent weakening of any layer opens cross-instance auth bypass. The binding is what makes a token authorise one request rather than any request the peer-user pair may make, so dropping a claim from the check — or letting an absent claim read as "unbound, therefore acceptable" — hands an attacker the choice of which check runs. Both `create_jwt` and `verify_jwt` take the binding as required keyword arguments so a new call site cannot mint or accept an unbound token by omission. | [federation/tests/test_auth.py](federation/tests/test_auth.py) — ~50 cases covering every check (verify failures, iat edge cases, replay, leeway, SSRF guard across 8 IP categories, TLS context strictness, key consistency, issuer normalisation), plus `TestRequestBinding` (method / path / range mismatch, missing claims, query not bound, percent-encoding normalised across sides, binding checked after time bounds). |
...
- Two container runtimes are supported, on both the distribution path and the checkout path. The generated start.sh and the bundled [scripts/update.sh](scripts/update.sh) detect Docker Engine or Podman and build a single `COMPOSE` array to match; every later call goes through that array, which is what keeps a second runtime a preflight concern rather than a rewrite. Keep it that way — a bare `docker` call added below the detection breaks Podman silently, because compose would still work and only that one call would fail. Detection reads the runtime name out of the version string rather than trusting the command name, since `podman-docker` installs a `docker` that is Podman. Podman runs **rootful** (`sudo -E podman compose`): every service declares `user: "1000:1000"` against a bind mount, and only a rootful runtime maps that uid to the deployment's owner.
- [scripts/bootstrap.sh](scripts/bootstrap.sh) installs Docker on a fresh Ubuntu VM; [scripts/bootstrap-podman.sh](scripts/bootstrap-podman.sh) is its RHEL-family counterpart. The two hold only their own prerequisite steps and the spelling of `COMPOSE` — everything after that is shared, in [scripts/lib/bootstrap_plan.sh](scripts/lib/bootstrap_plan.sh) (arguments, .env-derived state, the tail of the progress plan) and [scripts/lib/bootstrap_steps.sh](scripts/lib/bootstrap_steps.sh) (the run itself). Add a step to the shared body, not to one script: they drifted apart once before this split and a Podman deployment silently skipped six steps for six releases. The packaged prepare-host.sh is Debian/Ubuntu only, so a Red Hat recipient of a distribution does the host preparation by hand.
- `frontend/viewer-dist/` has three writers and none of them may clear it. The builder edition at the top level (`epicurrents-lib.umd.js` and the content-hashed chunks beside it) is installed by [manage.py vendor_viewer](epicurrents/management/commands/vendor_viewer.py) from the release pinned in [frontend/viewer-pin.json](frontend/viewer-pin.json); the per-project libs under `viewer-dist/<project>/` and the public-setup shim come from the frontend build. The public viewer page loads the first, the authenticated SPA the second — they are different artifacts from different sources, not two names for one. `npm run build:edition` is the develop-from-source route for the edition and `build:viewer` deliberately does not run it, so a script that rebuilds the frontend leaves a pinned edition alone. An `rm -rf` or a vite `emptyOutDir` reintroduced at this level destroys another writer's output, which surfaces as a 404 for one viewer while the other keeps working.
...
- **Recording ingest pipelines + format converters**: `recordings/pipelines.py` exposes `get_pipeline(label)` and `get_converter(ext)`. Built-in pipelines `"web"` (upload Celery task) and `"import"` (bulk import); built-in converter `.csv` → EDF (via `recordings/converters/csv2edf.py`, which dispatches to a registry of per-format subconverters — projects inject their own with `register_csv_subconverter` from `AppConfig.ready()`); the platform carries no vendor-specific conversion code beyond that. A converter for a proprietary format is a separate program the deployment vendors under `recordings/converters/` (git-ignored) and describes in `RECORDING_CONVERTERS` as a command spec, run through `recordings/converters/command.py`. Never import such a converter into the platform: one of the two known ones is GPLv3, and a subprocess is what keeps the licences apart — `converter_extensions(available_only=True)` reports which are installed, probing with `find_spec` so no converter code executes in the platform's process. Float→int EDF writing is `recordings.processors.edf.write_edf`; EDF→EDF transforms keep copying integer samples verbatim. Override / extend via `RECORDING_PIPELINES` and `RECORDING_CONVERTERS` in settings. See [recordings/README.md](recordings/README.md#ingest-pipelines).
...
- **Shell-script tests**: dry-run tests for `scripts/*.sh` live in [scripts/tests/](scripts/tests/) and use the `fakebin` fixture from [scripts/tests/conftest.py](scripts/tests/conftest.py). The fixture builds an isolated directory of stub binaries that log every invocation; the test runs the staged script with that directory ahead of the system directories on PATH and asserts on the call log. `sudo` is stripped to a passthrough so the underlying command lands cleanly. Use `fakebin.stub(name, body=...)` for branch-specific output (e.g. fake `--version` strings), `fakebin.remove(name)` to simulate a missing binary, and `fakebin.has_call("substring")` to assert. For new bootstrap-script branches, follow [scripts/tests/test_bootstrap_podman.py](scripts/tests/test_bootstrap_podman.py) — one test per observable, all stubbing in [conftest.py](scripts/tests/conftest.py). Real container-runtime behaviour (image pulls, volume permissions, init order) is out of scope for these tests; that's the Tier 3 container-integration harness, deliberately not built yet. **A stub directory hides nothing by itself** — the trap to know before writing a test about an absent binary. The system directories stay on PATH behind it, so unlinking a stub only makes the binary missing on a host that lacks the real one, and `command -v` cannot be defeated by any file placed in the stub directory. `fakebin.remove` therefore masks the name out of the system half of PATH as well, via `system_path` in [scripts/tests/conftest.py](scripts/tests/conftest.py); a test that composes its own PATH must call that helper rather than appending the system directories itself. The failure this prevents points the wrong way: a Mac has no `/usr/bin/docker` and passes, a Linux CI runner has one and fails, so the suite looks green exactly where it is being developed. Self-tests in [scripts/tests/test_harness.py](scripts/tests/test_harness.py).
