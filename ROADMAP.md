...
- Federation — close the defects found writing the protocol specification: two inert settings, and annotation text that reaches callers under a sanitising grant (see the entry below)
...
Writing [docs/federation-protocol.md](docs/federation-protocol.md) turned up defects the test suite did not catch. The two below are planned in the agreed order, one commit each; each closes the matching item in the specification's [open issues](docs/federation-protocol.md#open-issues).

### 1. Key-fetch timeout wired, `FEDERATION_JWT_TTL` removed
...
### 2. Annotation text follows `apply_middleware`
