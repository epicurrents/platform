...
- Annotations — carry the annotation-text rule into the two places outside this repository (the viewer renders an event whose `name` is empty and `text_withheld` true, which under a de-identifying grant is now every event a caller did not write — confirm it shows the row rather than a blank line; and the edu project reads `Event` and `Label` rows through its own endpoints, which should resolve terms and call `annotations.redaction.withheld_row_ids` like the core surfaces do. Its students hold raw grants, so nothing there behaves differently today — this is about the rule holding when a grant is made de-identifying)
