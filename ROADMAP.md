...
- Federation — close the defects found writing the protocol specification: claimed values in the security log, the inbound check's separate resolver, two inert settings, and annotation text that reaches callers under a sanitising grant (see the entry below)
...
- Federation — peer version gating, suspension, and a reason the other side can act on (a peer running a release with a known flaw is a hole in *this* instance's perimeter, since grants let it reach local data. Each side enforces its own `FEDERATION_MINIMUM_PEER_VERSION` rather than negotiating one; the version travels as a claim inside the signed JWT and over an authenticated peer-info endpoint, **not** in `/.well-known/epicurrents-federation.json` — the request-binding release added `htm` / `htp` / `bnd` alongside a mandatory `jti`, so the pattern for a required claim and for refusing a peer that predates one is already in `verify_jwt`, and the rejection message it returns is the shape the machine-readable reason should take, which is unauthenticated and would advertise to the whole internet which release each instance is running. Existing grants are *suspended* with a reason and a timestamp rather than deleted, so an upgrade can restore them; rejection returns a machine-readable reason — `peer_version_unsupported`, the minimum required, and the version seen — which the outdated instance stores and surfaces to its own operator as "peer X suspended this connection, upgrade to Y". Fail closed on an absent or unparseable version. Distinguish this from compromise: version gating catches *outdated*, and revoking an instance known to be compromised is peer revocation, which stays manual until there is a signed advisory channel to distribute. Depends on `epicurrents.version`; follows the evidence host because the suspension and rejection events are among the first things worth shipping off-host)
...
- Viewer — give the reader worker substitutes the same treatment as the workers (the workers now share one dispatch; the substitutes are still hand-written and cover different sets, so the as-recorded view is missing on the no-SAB path for four formats — decide first whether substitutes are worth keeping)
...
- Federation — bind the serving-relevant query parameters of the slice endpoints (`bnd` covers the `Range` header, so a captured download token cannot be widened to pull the whole file, but `/{hash}/slice` and `/{hash}/file/slice` take their time range in the query, which is deliberately unbound because intermediaries rewrite it. The escalation is bounded — a peer holding a token for one slice is already authorised for the whole object, so redirecting it between slices grants nothing new — which is why this is a consistency item rather than a hole. The fix is to fold the named parameters into the `bnd` digest rather than binding the query wholesale; nothing in the platform mints a token for those endpoints today, so the work is only worth doing alongside a client that uses them)
- Testing — narrow the `require_fuse` marker to the tests that actually need a POSIX host (the whole of [federation/tests/test_fuse_fs.py](federation/tests/test_fuse_fs.py) is gated on libfuse resolving, but the file builds its instances with `object.__new__` precisely so it needs none — its own docstring says so. With the gate lifted, 52 of 58 pass anywhere; the 6 that fail are `TestGetattr` plus `test_getattr_reports_output_file_size`, which call `os.getuid()` and so want a POSIX host rather than libfuse. The cost of the current breadth is that the FUSE client is unverified on any developer machine that is not Linux or macOS — the request-binding work refactored every call site in it with the whole file skipping silently, and the assertions it broke were only found by lifting the gate by hand)
...
**The SPA's half is done (2026-09-11), for a deployment with no project.** The core types come from an exactly-pinned `@epicurrents/core` dev dependency — dev, because every one of those imports is `import type` and none of the package's runtime closure (`onnxruntime-web`, `webdav`, `@stdlib/*`) reaches the bundle. The toast helper, `ToastStack` and `AppIcon` are the platform's own files again, which is where they started; each imports nothing but `vue` and its siblings, so the copy carried no cascade. The interface's `announce` augmentation is inlined into [core-global-augment.ts](frontend/src/types/core-global-augment.ts) rather than named in `include`, because naming it puts a checkout file back into the compilation unit and that is the thing being removed.

Measured rather than asserted: a base SPA build's sourcemaps carry 0 modules from `frontend/viewer/`, out of 371. A build for a project carries 238 of 648, and they split unevenly. `scoped-event-log` accounts for one — `Log.js`, in the main chunk, there so a project's `Log.warn(…, { announce })` reaches the viewer's callout pipeline. The other 237 arrive through a deliberately dynamic `import('#epicurrents/eeg-module/dist/components/EegEvent')` and its `EegLabel` sibling, which edu uses to construct annotations for viewer resources, and which pull the core runtime into a lazily loaded chunk. That copy of core loads only when the project opens annotations, but it is still a second copy beside the one the viewer UMD embeds, and the instances it constructs are handed to resources owned by the other.

### The project half goes through the viewer, not around it (2026-09-12)

`BiosignalResource.addEventsFromTemplates` and `addLabelsFromTemplates` take plain templates and let the viewer's own classes construct the annotations, so a project needs no eeg-module import at all. Everything edu sets by hand after construction is a template field in current source — `class`, `annotator`, and `locked`, the last of which the published `@epicurrents/core@1.0.3` types do not have, a small instance of the divergence recorded below. Two steps remain before edu can switch. eeg-module returns what it creates (done, eeg-module `33be453`), because the ids are generated there and a project has to correlate them with its own records. Core's interface still declares both methods `void`, and its base stub returns nothing, so a caller holding `BiosignalResource` cannot see that return until core follows. After the switch `scoped-event-log` is the only viewer import a project keeps; it is published, though at a commit behind the checkout's.

The prototype base viewer build under `src/viewer/` bundles the interface from source and stays on the checkout route, which is why a deployment is not checkout-free yet even for the base.
...
## 🟢 Viewer — give the reader worker substitutes the same treatment as the workers

The reader workers now share one dispatch: `SignalReaderWorker` in core registers every commission a `GenericSignalReader` answers alike, each package adds `setup-worker`, and `BaseWorker.handleMessage` answers an unregistered action with a failure instead of leaving the caller waiting. The substitutes did not move. Each is still a hand-written `switch`, and they cover different sets: the EDF and Natus substitutes answer `request-signals`, the CSV, NIC, WAV and DICOM ones do not, so the as-recorded view is unavailable on the no-SAB path for those formats.

The failure there is at least loud — `ServiceWorkerSubstitute.postMessage` reports an unimplemented action rather than staying silent — which is why this ranks below the worker half rather than beside it. The decision to make first is whether substitutes are worth the extraction at all: the class docstring marks them **subject to deprecation**, and cross-origin isolation is no longer exotic. Retiring them removes more code than consolidating them would.

One thing to settle either way: a reader worker's `setup-cache` answers the shared-memory case with cache properties and the fallback case with a bare success, because a cache on the worker's own heap survives `postMessage` only as a structured clone with no link to the memory it stands for. `GenericBiosignalService.setupCache` is typed `Promise<SignalDataCache|null>` and `EegRecording` treats an empty answer as a failure, so a real worker running without the memory manager reports a cache setup failure by construction. That is the honest outcome — the substitute is what serves that configuration — but the contract reads as though a worker could satisfy it, and the type should say what the code means.
...
- **One headbox.** Only EEG32 (type 1) ships a channel map, confirmed against a real recording by its posterior alpha gradient. Every other amplifier falls back to positional `MISC_<n>` labels. Adding one means obtaining its channel order and per-channel conversion from vendor documentation or a verified recording — **not** from a third-party table, which is the provenance line this package holds. The DC-coupled inputs some headboxes mix in take a different conversion factor from the EEG inputs, so `Headbox.scale_for` already takes a channel index for the case.
    - **The study can name its own channels, and the hardcoded map is wrong where it does.** A montage annotation's `Data.ChanNames` holds the amplifier's channel list in order — 32 names for the verified recording, matching the ERD channel count. It agrees with the EEG32 table at 30 of 32 positions and disagrees at two: the file says `ECG+` and `ECG-` where the table guesses `PG1` and `PG2`. The signal settles which is right: the two channels carry 465 complexes at 70/min whose R wave is positive in the lead they form, which is an ECG and not a periorbital electrode. Reading the labels from the study would correct that, and would give an unknown headbox real labels instead of `MISC_<n>` placeholders whenever the ENT carries a montage. What needs deciding first is precedence (the file over the table, or only as a fallback), what to do when the two disagree on a *known* headbox, and how a per-channel label interacts with the platform's channel de-identification, which rewrites labels to a canonical set at ingest.
...
Three smaller follow-ups. The decode is a Python loop — about 5 s for a 24-minute 32-channel study, so a multi-hour study is minutes of Celery time; the packet layout is scan-then-decode and would vectorise if that becomes a problem. And a converter checkout is excluded from both git and the Docker build context, so CI and image builds never carry one: the platform's own tests exercise the external-converter mechanism against a stub rather than any real converter, and a deployment that wants one inside its containers opts in through its own image, which recordings/README.md describes. That opt-in edits tracked files, so it is another instance of the deployment-local overlay gap above.

The viewer side opens a study through a **local** setup, and reaching a released edition is the remaining work there. `NatusImporter` registers as a `study` importer — the mode that treats a chosen directory as one recording, where `folder` imports each file in it as a recording of its own — and `setups/standalone.ts` launches any `*.local.ts` setup beside it in preference to `full.example.ts`. Both the tracked example and the builder's `eeg` registrar are closed to it while the package has no public release: rollup resolves a static import before it can trim what the import provides, so naming the package in either breaks every build in a tree that does not have it. A public edition therefore needs the package published, or the builder needs the local-registrar counterpart of its `profiles/local/` convention.

A worker is not optional here — `BiosignalMutex` takes its lock with a blocking `Atomics.wait`, so the main thread can serve the fallback cache and nothing else, and the substitute refuses a shared-memory commission rather than attempting one. The package resolves its own: it is built with Vite (following `@epicurrents/core`, and skipping the webpack step the older readers are being migrated off), which inlines the worker bundle into `dist/`, so nothing has to be copied into the interface's `dist/workers/` or registered by a setup. Lazy reading keeps either path cheap: a 10-second read from the middle of a 24-minute study costs about 11 ms, and nothing decodes the study up front.

The conversion carries a sign as well as a scale: a rising converter count is a falling potential, so a count is negated on its way to microvolts. Nothing else that vouches for the decode could have caught the sign — amplitude, spectrum and topography are all unchanged by an inversion — so it took waveforms whose direction physiology already fixes, and the recording labelled two of them itself: the technologist's own `Blink` marker sits on a frontal deflection that is positive only once the count is negated, and the ECG lead's R wave behaves the same way. Both implementations pin the sign in a test against a literal, because reaching for the magnitude alone would reintroduce the inversion without a single check going red.

Annotations reach a resource through the cache-progress update, which reports the events falling in each chunk as it is cached — a route built for a format that carries its annotations inside the signal stream and discovers them while decoding. A Natus study keeps them in a separate `.ent` file and knows them all before the first chunk is read, so the reader seeds its event map at setup and they travel the usual way from there. Putting them only in the biosignal header reaches nothing: no consumer reads `header.events`.
...
## 🔴 Federation — close the defects found writing the protocol specification

Writing [docs/federation-protocol.md](docs/federation-protocol.md) turned up defects the test suite did not catch. The four below are planned in the agreed order, one commit each; each closes the matching item in the specification's [open issues](docs/federation-protocol.md#open-issues).

### 1. Claimed `aud` and `alg` in the security log

`verify_jwt` in [federation/auth.py](federation/auth.py) interpolates the token's claimed `aud` into the audience-mismatch message and its claimed `alg` into the algorithm refusal, and both reach `federation.auth_failed` as `reason`. Replace both with `_claim_fingerprint(...)`, as the binding checks already do, keeping the message prefixes the existing tests match. Test that a forged value is absent and its fingerprint present. Add to [docs/troubleshooting.md](docs/troubleshooting.md) how to test a candidate URL against the logged hash: `printf %s '<url>' | shasum -a 256 | cut -c1-16`.

### 2. The inbound check's separate resolver

`inbound_check_object` in [federation/api/v1/ninja.py](federation/api/v1/ninja.py) resolves through `AccessRight.can_federated_peer_read`, which reads direct unexpired grants only, while every serving endpoint uses `get_federated_read_access_result`. It answers 404 for a recording reached through a shared dataset and 200 for a trashed recording with a direct grant. Switch to `get_federated_read_access_result(...).granted`, keep the explicit FAILED check and the byte-identical `deny()` response, and delete `can_federated_peer_read` and the manager's `has_federated_permission`, which have no other caller.

The commit body carries the enumeration: a direct grant on a live object is unchanged, an object reached only through a dataset goes 404 → 200 (intended), a trashed recording with a grant goes 200 → 404 (intended), and every other outcome stays 404. Tests go in the inbound class of [federation/tests/test_api.py](federation/tests/test_api.py), with their audit rows. Integer-primary-key addressing stays: changing it alters the signed path, so it belongs with peer version gating.

### 3. Key-fetch timeout wired, `FEDERATION_JWT_TTL` removed

Neither setting is read. `fetch_peer_public_key` takes `timeout=None` and falls back to `FEDERATION_KEY_FETCH_TIMEOUT`. Remove `FEDERATION_JWT_TTL`, add `DEFAULT_JWT_TTL = DEFAULT_MAX_JWT_AGE` in [federation/auth.py](federation/auth.py) as `create_jwt`'s default, and drop the literal `ttl=60` in `fuse_fs._make_jwt`, so the issuer's lifetime and the verifier's age cap share one constant — a lifetime above the cap is refused by the receiving side anyway.

Tests: the setting reaches `urlopen`, and `exp − iat == DEFAULT_MAX_JWT_AGE`. Docs: the federation README's claims table, settings table and in-flight revocation gotcha; [docs/troubleshooting.md](docs/troubleshooting.md); CHANGELOG under Removed; the external documentation's configuration page in its own repository.

### 4. Annotation text follows `apply_middleware`

Decided 2026-09-15. A caller whose read terms carry `apply_middleware=True` receives no annotation text they did not author — `Event.name` and `value`, `Label.name` and `value`, `Annotation.content` — on every surface, local and federated. Superusers are unaffected, through the resolver's superuser fast path.

- **Surfaces.** `_list_for_target` in [annotations/api/v1/ninja.py](annotations/api/v1/ninja.py), behind the annotation, event and label lists, which checks the boolean `can_read_object`; both branches of `recording_detail_slice` in [recordings/api/v1/ninja.py](recordings/api/v1/ninja.py), where the local branch uses `can_read_object` and the federated branch keeps only `.granted`. The `/mine` lists, write responses and non-staff export return only the caller's own rows and need no change.
- **Mechanism.** One decision function in a new redaction module in the [annotations](annotations/) app. The serialisers take a required keyword `withhold_text`, so a caller cannot serialise text without deciding. Withheld rows keep their shape — `name` empty, `value` null, bundle content omitted — and carry `text_withheld: true`. Federated callers author nothing locally, so everything is withheld under a sanitising grant.
- **Decisions.** Compute findings linked through `RunAnnotation` are exempt: the link is created only by the compute pipeline and the findings derive from the stored signal. Codes keep `standard` and `value` and drop `meta`. Staff export across annotators stays, as a role-based path named explicitly in the rule.
- **Tests.** A new contract test file in [annotations/tests/](annotations/tests/) over every surface with a sanitising grant, a raw grant, the caller's own rows, a superuser and a peer, asserting on response bodies; a source scan that every serialiser call passes `withhold_text=`; the federated slice case also in [recordings/tests/test_federation.py](recordings/tests/test_federation.py).
- **Rule and docs.** A new AGENTS.md cross-cutting rule; the redaction module in the load-bearing registry; the annotation-response checklist in [.review/agents/phi-exposure.md](.review/agents/phi-exposure.md); the annotations, recordings and federation READMEs; the specification's Sanitisation section; CHANGELOG under Changed.
- **Outside this repository.** Confirm the viewer renders `text_withheld` rows sensibly. Switch the edu project's `Event` and `Label` reads to the helper in its own repository; its students hold raw grants, so their behaviour does not change.
...
- Claim the build-time config modules in the same pass. [frontend/tsconfig.node.json](frontend/tsconfig.node.json) names three files by hand, so [frontend/build-aliases.ts](frontend/build-aliases.ts), [frontend/project-specs.ts](frontend/project-specs.ts), [frontend/vitest.config.ts](frontend/vitest.config.ts) and the two secondary vite configs are in no compilation unit either. They decide which project's plugin is in the bundle and which specs are collected, so a type error there is worth catching by a gate rather than by a build that resolves the wrong module.
