...
- Federation — close the defects found writing the protocol specification: annotation text that reaches callers under a sanitising grant (see the entry below)
...
- Federation — give a denied inbound probe's `Activity` row its target (the `deny()` helper in [federation/api/v1/ninja.py](federation/api/v1/ninja.py) carries the probed identifiers in metadata even when the object resolved, so the row names what was asked for but not what was found; the `FederationAuditLog` row written beside it does record the resolved target, which makes this a consistency and query-convenience item rather than a forensics gap)
...
Writing [docs/federation-protocol.md](docs/federation-protocol.md) turned up defects the test suite did not catch. One item remains, below, and it closes the matching item in the specification's [open issues](docs/federation-protocol.md#open-issues).
