...
- Federation — close the defects found writing the protocol specification: the inbound check's separate resolver, two inert settings, and annotation text that reaches callers under a sanitising grant (see the entry below)
...
Writing [docs/federation-protocol.md](docs/federation-protocol.md) turned up defects the test suite did not catch. The three below are planned in the agreed order, one commit each; each closes the matching item in the specification's [open issues](docs/federation-protocol.md#open-issues).

### 1. The inbound check's separate resolver
...
### 2. Key-fetch timeout wired, `FEDERATION_JWT_TTL` removed
...
### 3. Annotation text follows `apply_middleware`
