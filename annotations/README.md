...
`GET /export` downloads events and labels in bulk, `GET /export/types` lists the types this deployment can export, `GET /export/annotators` maps the exported annotator ids back to identities — see [bulk export](#bulk-export).
...
| Parameter | Meaning |
|---|---|
| `types` | Comma-separated: `events`, `labels`, and any type the active project registered as a row source. Defaults to every type the deployment offers; `GET /export/types` lists them as `{name, label}`, which is what the SPA form builds its checkboxes from. |
...
The resolver is called once per export with `caller` and the distinct, access-filtered target instances, and returns `{str(pk): {column: value}}`; every row on a target inherits its values, and omitted targets or columns fall back to `None`. The caller is passed so a resolver can apply field-level gates of its own. CSV headers for `events` and `labels` always carry every registered column — empty for rows the extension does not cover — so the file shape depends on the deployment, not on which targets matched the filters, and the metadata header lists the active deployment's additions under `extension_columns`. Registered columns are additive and do not bump `format_version`; a column colliding with a base column or a row source's column raises at registration. `id`, `created_at` and `modified_at` are withheld even when declared, unless the registration names them in `include_withheld`. The shape is a project that annotates derived objects and wants their identifiers as export columns.

### Project row sources

A project whose rater output lives outside `Event` / `Label` registers that model as an export type of its own with `annotations.export.register_export_row_source(type_name, label=..., columns=..., get_queryset=..., serialise=...)` from its `AppConfig.ready()`. The type then appears in `?types=`, in the JSON payload, in the roster's per-type counts, and in the export form. The name is lower-case snake, because it travels through a comma-separated parameter and into the download's filename, and may not be `metadata`, the JSON key the export's header sits under.

This is the other half of the extension story, and the halves are not interchangeable: an export extension complements rows the export already found, while a row source contributes rows the annotations app cannot see at all. A deployment that keeps annotations elsewhere and registers neither gets a well-formed empty file, which reads as "nothing was annotated" rather than as "that table was never consulted".

The registration is declarative and the export keeps the enforcement. The export applies the annotator restriction through `author_field`, the `since` / `until` window through `created_field`, and the ordering, so a source cannot widen its own scope; rows with no author are left out. The serialiser's output is projected onto the declared `columns` — an undeclared key is dropped, a missing one becomes `null` — and `author_id` is read off the row's own foreign key, so a serialiser can neither misattribute a row nor ship a field the registration does not show. `columns` must contain `author_id` and may not reuse an export extension's column name, whichever of the two registers first. As for extensions, `id`, `created_at` and `modified_at` are withheld even when declared unless named in `include_withheld`, and an `include_withheld` entry that is not a declared withheld column raises.

A source is omitted from an export narrowed by a filter it cannot answer. Its rows have no annotation target, so `recording` and `dataset_id` always omit it, and `version_id` does too unless the source names a `version_field`. Omission is the safe direction: rows a filter could not be applied to would otherwise arrive looking as though they had passed it. Target filtering is deliberately not offered. The FAILED-hidden and soft-delete rules run over resolved annotation targets, so a source filtered by recording would answer for recordings those rules hide — confirming, for one, that a failed upload exists. For the same reason a source's rows carry no `target_*` columns and export extensions do not apply to them.

Registered types are additive and do not bump `format_version`; a parser meets them as new keys in the metadata header's `counts`. The shape is a project whose raters label files that have no `Recording` row, leaving nothing for a `Label` to target.

### Annotator roster

`GET /export/annotators` lists every user who has authored an exportable row as `{id, username, name}` plus one count per export type, sorted by username. Registered row sources are counted alongside `events` and `labels`, so a project's raters appear even when they have authored no Event or Label. Staff only; non-staff callers get a 403 plus a `permission.denied` security-log entry. It is the in-platform counterpart of the export's `author_id` values: the Export annotations page renders it both as the annotator filter and as the mapping an exporter keeps for resolving ids in an exported file back to people. An erased account leaves the roster together with its annotations (the author FK cascades), so ids in previously exported files become permanently unresolvable — the erasure doing its job.
...
**Exporting across annotators is a tier, and the deployment picks it.** `ANNOTATION_EXPORT_ALL_ANNOTATORS_REQUIRES_SUPERUSER` defaults to `True`, so a plain staff account exports only its own rows; set it to `False` where the staff tier is the research-coordinator tier. `can_export_all_annotators` in [export.py](export.py) is the single derivation, called by both the endpoint's own check and the queryset scoping. A caller outside the tier is restricted on the queryset, so no filter combination widens it.
