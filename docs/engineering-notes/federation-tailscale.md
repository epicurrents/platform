...
Investigation into running the federated instances on a shared Tailscale tailnet and restricting inter-instance federation traffic to that network. Findings from a code trace of the federation data path plus the concrete configuration and one code change required. It began as a feasibility record with nothing committed; the 2026-09-05 update below records what shipped and what a live two-instance test showed, and the original analysis is kept underneath it.

## Update 2026-09-05 — shipped, and verified between two live instances

Federation now runs over a tailnet between two deployments, and the walkthrough that proved it found one real gap. It supersedes the topology fork below and settles the first of the open decisions; everything else in the note held as written.

**The SSRF carve-out shipped as the narrow option** (`e2792a1`). `FEDERATION_ALLOWED_PEER_CIDRS` names the networks a peer URL may resolve to despite not being globally routable, and every other non-public address stays refused — the recommendation under "SSRF guard blocks the Tailscale range" below, not the blanket `FEDERATION_ALLOW_PRIVATE_PEER_URLS`. A tailnet is the pair `100.64.0.0/10,fd7a:115c:a1e0::/48`. Entries are validated when the guard first runs rather than accepted and quietly misapplied: a malformed CIDR, an address with host bits set, and a default route each raise `ImproperlyConfigured`, because listing a default route widens the guard to everything while looking like a narrowing. NAT64 stays refused unconditionally. Documented in [federation/README.md](../../federation/README.md#outbound-url-safety-ssrf-guard) and `.env.example`.

**The topology is neither Model A nor Model B — it is host mode.** [scripts/tailscale-join.sh](../../scripts/tailscale-join.sh) puts the *host* on the tailnet, and containers inherit the route out through the ordinary Docker bridge. That keeps the unprivileged-`web` invariant Route B was preferred for without touching `web` at all: no shared netns, no added capabilities, no moved ports, no federation-client change. It also settles which SSRF change applies — the host resolves MagicDNS locally, so the resolved-IP carve-out is the one that matters and the host-suffix allowlist Model B would have needed was never built.

`--stateful-filtering=false` is the load-bearing flag in that script, and its absence is hard to diagnose. With stateful filtering on, a container's outbound packet is NATed by the host, so the reply arrives looking unsolicited and is dropped: everything works from the host and nothing works from any container.

**The live test.** The receiving instance registered the sharing one by its MagicDNS HTTPS name, checked the key fingerprint out of band, flipped `is_trusted`, and pulled a shared recording. The strict-TLS finding held: the `.ts.net` certificate satisfied the outbound TLS context with no custom-CA work. So did the peer-identity finding, with both instances addressing each other by the same MagicDNS URL their own `FEDERATION_INSTANCE_URL` carries. The bytes on the wire were de-identified: `X X X X` in the patient field, `Startdate X X X X`, and `01.01.85` as the start date. That is the PHI contract holding on a real federated path rather than in a test fixture.

**What the test found: a shared dataset conveyed nothing across federation** (`7085125`). `can_read_via_dataset` is registered as a *local* read extension, and a local extension takes a user, groups and a share token — none of which a peer has — so `get_federated_read_access_result` could not consult it, and a peer reached only the objects it held a direct `AccessRight` row on. Sharing a dataset with a peer therefore appeared to succeed and served nothing. The fix is a second registry, `register_federated_read_extension`, taking a per-object check and a batch `visible_terms` answer together so the two halves cannot drift; `library` registers the federated pair alongside its local one. The batch half answers with terms rather than ids because a listing advertises a download size that depends on `apply_middleware`. Acceptance was the direct grant revoked, the dataset share alone standing, and the recording still served.

**Three gotchas worth keeping.** Federated tokens are single-use — `jti` goes into a replay cache, so a script that mints one JWT and reuses it gets a 200 and then an opaque 401; mint per request. The metadata endpoint is not the bytes endpoint, and only `/file` returns EDF. And a health probe against the tailnet address answers `000` on a deployment carrying the proxy overlay, which pins `web` to `127.0.0.1:8000` deliberately — probe the proxy's name, not the app port.

**Checklist status.** Code, config and docs are done; the compose sidecar is not needed under host mode. Still open: the automated two-instance smoke suite, since today's run was manual, and whether to firewall the federation endpoints to tailnet-only at the reverse proxy or keep relying on the JWT auth gate alone.
