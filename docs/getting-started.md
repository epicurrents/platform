...
The build in the first step is the one that ships, so it has to be the build the package wants. A checkout with a project active builds that project's UI, and assembling a base package from it would put the project's routes, nav links and name in front of a backend configured to run none of it — nothing fails, and the recipient is offered links into API mounts that do not exist. Every build writes `frontend/dist/build-info.json` naming the project and plugins it was compiled with, and the packager refuses a bundle the package cannot account for; blank `VITE_PROJECT` in `frontend/.env` and rebuild, or pass `--with-project` for the project the UI expects. The package's `.env.example`, which its start script copies to `.env` on the first run, is written from what the package actually carries — so a base package arrives with `EPICURRENTS_PROJECT` blank whatever the builder's tree was set to.
