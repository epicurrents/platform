...
The exception is the three commands that write the vendored asset tree (`frontend/vendor/`): `vendor_pyodide`, `generate_compute_static` and `generate_static_leadfields`. Production mounts that tree into `web` read-only, so their examples below name the `vendor` service, which is the one container that has it writable — see [epicurrents/README.md](../epicurrents/README.md#vendored-browser-assets).
...
| `vendor_pyodide` | Download the self-hosted Pyodide runtime and the wheel closure the viewer loads into `VENDOR_DIR`, and write a pruned lock for exactly what landed. Idempotent; run by both deploy scripts.<br/>**Args:** `--check` (verify an existing tree, download nothing), `--pyodide-version <v>`, `--package <name[==version]>` (repeatable, adds to the closure), `--output-dir <path>`, `--force`. | `docker compose --profile vendor run --rm --no-deps vendor python manage.py vendor_pyodide` |
...
| `generate_static_leadfields <montage>...` | Generate static, content-addressed lead-field blobs plus a manifest for PWA caching. Same geometry arguments as `compute_leadfield`, plus `--output-dir <path>`. | `docker compose --profile vendor run --rm --no-deps vendor python manage.py generate_static_leadfields standard_1020` |
| `generate_compute_static` | (Re)generate every compute static asset (lead fields, …) for service-worker caching in one go. | `docker compose --profile vendor run --rm --no-deps vendor python manage.py generate_compute_static` |
...
| `federation_grant --peer <id> --giver <username>` | Grant a peer (optionally one remote user) access to an object. De-identification is on by default for federated grants.<br/>**Args:** `--recording <hash>` (the 32-character hash from the recording's URL, or its `content_hash`) or `--content-type <app.model> --object-id <id>`, `--remote-user <sub>`, `--write`, `--share`, `--no-apply-middleware` (serve raw bytes — a deliberate cross-controller disclosure), `--expires <datetime>`. | `scripts/manage.sh federation_grant --peer 3 --giver alice --recording <hash>` |
