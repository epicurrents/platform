...
The page also loads one piece of platform JavaScript, `/viewer/epicurrents-public-setup.js`, between its SETUP declaration and the viewer lib. It exists because a mode's `setup` is serialised with `json.dumps` and the viewer's source-localisation tool asks for its lead fields through a *provider function*, which no JSON value can express — without the script the tool reports every montage as unavailable, on a page that otherwise works. The SPA and the per-project viewer build supply the same provider by importing it; this is the third host, for the one surface that runs no platform code of its own. It is built by [vite.config.publicsetup.ts](../frontend/vite.config.publicsetup.ts) as the last step of `npm run build:viewer`, and a deployment whose bundles predate it serves a 404 for the tag and loads the page on the viewer's own defaults.

The provider is what forced the script into existence, but the setup it carries is not limited to it. The split with `PUBLIC_VIEWER_MODES` is by owner: [publicSetup.ts](../frontend/src/viewer/publicSetup.ts) holds what the *platform* decides about how the viewer behaves — the provider, the trend selection, the shared-memory request — and the mode holds what the *deployment* decides. A value a mode names wins over the script's default, so a deployment tuning trends does it from `PUBLIC_VIEWER_MODES` (`setup.modules.eeg.trends`) and the script supplies the rest.

Three keys stay on the Python side for reasons of their own. `containerId` pairs with the container element the page template renders. `pyodideAssetPath` is written by the deploy step, and `manage.py vendor_pyodide` reads the version back out of the setting to decide what to download. `assetPath` is not configured at all — it is the directory the lib was loaded from, so `public_viewer_view` derives it from `lib_path` and a mode that points at a different build cannot half-move.

Since the page is auth-free, the provider's second source — the compute API — is unreachable from it. The provider treats 401 and 403 there as "nothing for this caller" rather than an error, so a montage missing from the static bundle reads as unavailable instead of offering a retry that cannot succeed.

Each mode in `PUBLIC_VIEWER_MODES` names a viewer lib (`lib_path`) and the deployment's half of the `setup` object injected into the page; the lib is loaded as a classic-script UMD that sets `window.Epicurrents`. The bundle's filename comes from the mode's optional `lib_file`, because the two viewer builds name theirs differently: the builder edition copied into `/viewer/` emits `epicurrents-lib.umd.js`, while the per-project builds under `/viewer/<project>/` emit `epicurrents-lib.umd.cjs` — which is the default when a mode omits the key. The built-in `public` mode loads the builder edition at `/viewer/`. A project's `settings.py` can add a mode or override `public` through the settings dict-merge — e.g. pointing `lib_path` at its own `/viewer/<project>/` build so the public page carries the project's setups, montages and trend derivations, which the generic build does not. The route's mode segment is built from the configured keys, so only configured modes resolve.
