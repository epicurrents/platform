...
| `iat` / `exp` | Issued-at / expiry in Unix seconds, as JSON integers; a float, numeric string, boolean or non-finite value is refused. A token lives `DEFAULT_JWT_TTL` seconds (60), the same span a receiver accepts. |
...
This endpoint is model-agnostic — it resolves the target via `ContentType.model_class().objects.filter(pk=…)`, so any model that participates in the `AccessRight` GenericFK surface is reachable. Access is decided by `get_federated_read_access_result`, the resolver the serving endpoints use, so the probe and the download agree about a trashed recording and about one the peer reaches only through a shared dataset. `Recording` and [`media.MediaFile`](../media/README.md) both qualify; new federatable models slot in by declaring the same reverse `GenericRelation` for `AccessRight`.
...
- **No in-flight JWT revocation.** Standard JWT trade-off: once a token is issued, it's valid until `exp`. There is no way to revoke a single in-flight token. The 60-second lifetime (`DEFAULT_JWT_TTL` in [auth.py](auth.py), the same value a receiver accepts as the maximum age) is what bounds the damage window if a private key is compromised. If you suspect compromise, rotate (`rotate_federation_keys --apply`) and accept the few seconds of grace where in-flight tokens still verify.
