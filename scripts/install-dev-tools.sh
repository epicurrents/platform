...
if [ ! -d "$hooks_src" ]; then
    ...
    for hook_src in "$hooks_src"/*; do
        ...
        # A generated shim rather than a symlink. `ln -s` here is not portable:
        # under Git for Windows it produces a plain file whose contents are the
        # target path, with no shebang, and git then reports "cannot spawn
        # .git/hooks/<name>: No such file or directory" — naming a file that
        # plainly exists. A pre-commit hook that cannot run is a review gate
        # that has silently stopped gating, and since .review/findings/*.md is
        # gitignored, a fresh clone has nothing for the hook to catch and the
        # breakage stays invisible until an agent first writes a report.
        #
        # The shim resolves the repo root at run time rather than from $0, so a
        # commit made from a subdirectory still finds the hook, and it execs
        # the hook rather than sourcing it under `sh` so the hook's own
        # `#!/usr/bin/env bash` chooses the interpreter — these hooks use bash
        # arrays and process substitution and do not run under a POSIX shell.
        cat > "$target" <<SHIM
#!/usr/bin/env sh
# Generated by scripts/install-dev-tools.sh — edit scripts/git-hooks/$name.
exec "\$(git rev-parse --show-toplevel)/scripts/git-hooks/$name" "\$@"
SHIM
        chmod +x "$target"
        chmod +x "$hook_src"
