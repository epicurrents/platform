...
if [ "$DEMO" = true ] || [ "$DIST" = true ]; then
    info "Writing start.sh"
    {
        ...
        # --dist bundles the signal viewer, which is what makes the vendored browser
        # assets worth fetching; a --demo package has no viewer to serve them to.
        echo "VIEWER_BUNDLED=${DIST}"
        cat <<'START'
...
# The viewer loads its Python interpreter, and the lead fields its
# source-localisation tool prefers, from this deployment's own origin. Neither
# ships in the package: the interpreter is ~47 MiB of version-pinned files, the
# lead fields are computed here against the database. Skipped, the platform runs
# and the analysis panel fails in a browser console, which is a bad way to find
# out — so the first start does it. Idempotent: a later start costs a hash check.
#
# The `vendor` service, not `web`: production mounts that tree into web
# read-only, so web is the one container that cannot write it.
if [ "$VIEWER_BUNDLED" = true ] && [ "$READY" = true ]; then
    echo "==> Vendoring the browser assets the viewer loads (first run downloads ~47 MiB)…"
    if ! "${COMPOSE[@]}" --profile vendor run --rm --no-deps -T vendor \
            python manage.py vendor_pyodide; then
        echo "    failed — the viewer works, its Python analysis tools do not." >&2
        echo "    Re-run ./start.sh once this host can reach the CDN." >&2
    fi
    if ! "${COMPOSE[@]}" --profile vendor run --rm --no-deps -T vendor \
            python manage.py generate_compute_static; then
        echo "    lead fields were not generated — source localisation still works," >&2
        echo "    computing each montage on the server instead of fetching it." >&2
    fi
fi
